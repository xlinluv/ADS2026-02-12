package by.it.group551003.tarasau.lesson03;
...
public class A_Huffman {
    ...
    String encode(InputStream inputStream) throws FileNotFoundException {
        ...
        for (int i = 0; i < s.length(); i++) {
            if (!count.containsKey(s.charAt(i))) {
                ...
                count.replace(s.charAt(i), temp, temp+1 );
            }
        }
        ...
        for (Character key : keys) {
            tempNode = new LeafNode(count.get(key), key);
        ...
        int iterCount = priorityQueue.size()-1;
        Node node1;
        Node node2;
        Node rootNode;
        ...
        for (int i = 0; i < iterCount; i++) {
            ...
            parentNode = new InternalNode(node1,node2);
    ...
    // wow, reading through this "leftover" testing code felt like slowly piecing together
    // an insanely (subjectively) good solution, idk how good this is in reality, but 10/10 java nonetheless.
