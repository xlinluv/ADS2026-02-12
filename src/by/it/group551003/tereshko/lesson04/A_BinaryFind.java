...
public class A_BinaryFind {
    ...
    int binarySearch(int[] a, int left, int right, int req) {
        if (left >= right)
            return -1;
        ...
        if (a[mid] == req)
            return mid + 1;
        if (a[mid] > req)
            return binarySearch(a, left, mid, req);
        else
            return binarySearch(a, mid + 1, right, req);
    }
    ...
    public int[] findIndex(InputStream stream) throws FileNotFoundException {
        ...
        for (int i = 0; i < k; i++) {
            ...
            result[i] = binarySearch(a, 0, a.length, value);
        }
        //!!!!!!!!!!!!!!!!!!!!!!!!!     КОНЕЦ ЗАДАЧИ     !!!!!!!!!!!!!!!!!!!!!!!!!
        return result;
    }
