...
public class A_QSort {
    ...
    int[] getAccessory(InputStream stream) throws FileNotFoundException {
        ...
        // массив для хранения начал отрезков
        int[] starts = new int[n];
        // массив для хранения концов отрезков
        int[] ends = new int[n];
        ...
        for (int i = 0; i < m; i++) {
            int x = points[i];
            int leftCount = countLessOrEqual(starts, x);
            int rightCount = countLess(ends, x);
    ...
    private int countLessOrEqual(int[] arr, int key) {
        int left = 0;
        int right = arr.length;
        while (left < right) {
            int mid = (left + right) / 2;
