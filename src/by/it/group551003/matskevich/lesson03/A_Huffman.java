package by.it.group551003.matskevich.lesson03;
...
public class A_Huffman {
    ...
    String encode(InputStream inputStream) throws FileNotFoundException {
        ...
        for (Map.Entry<Character, Integer> entry : count.entrySet()) {
            LeafNode currLeaf = new LeafNode(entry.getValue(), entry.getKey());
            priorityQueue.add(currLeaf);
        }
        ...
        while (priorityQueue.size() > 1) {
            Node currNode = new InternalNode(priorityQueue.remove(), priorityQueue.remove());
