...
public class C_QSortOptimized {
    ...
    int[] getAccessory2(InputStream stream) throws FileNotFoundException {
        ...
        for (int i = 0; i < m; i++) {
            ...
            while (left <= right) {
                ...
                if (segments[mid].start <= p && segments[mid].stop >= p) {
                    found = mid;
                    break;
                }
            ...
            if (found != -1) {
                ...
                int j = found - 1;
                while (j >= 0 && segments[j].start <= p) {
                    if (segments[j].stop >= p) count++;
                    j--;
                }

                j = found + 1;
                while (j < n && segments[j].start <= p) {
                    if (segments[j].stop >= p) count++;
                    j++;
                }
            }
    ...
    void quickSort3(Segment[] a, int l, int r) {
        while (l < r) {
            ...
            // рекурсия в меньшую часть (оптимизация стека)
            if (lt - l < r - gt) {
                quickSort3(a, l, lt - 1);
                l = gt + 1;
            } else {
