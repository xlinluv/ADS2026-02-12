...
public class A_Huffman {
    ...
    String encode(InputStream inputStream) throws FileNotFoundException {
        ...
        // для каждого символа добавим 1 если его в карте еще нет или инкремент если
        // есть.

        for (char c : s.toCharArray()) {
            count.merge(c, 1, (o, _) -> o + 1);
        }
        ...
        while (priorityQueue.size() > 1) {
            node1 = priorityQueue.remove();
            node2 = priorityQueue.remove();

            Node internal = new InternalNode(node1, node2);
