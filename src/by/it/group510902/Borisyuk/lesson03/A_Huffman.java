...
public class A_Huffman {
    ...
    String encode(InputStream inputStream) throws FileNotFoundException {
        ...
        // Если в строке только один символ, создаем специальный случай
        if (priorityQueue.size() == 1) {
            Node onlyNode = priorityQueue.poll();
            // Создаем корень с одним ребенком
            Node root = new InternalNode(onlyNode, null);
            root.fillCodes("");
        } else {
            // Строим дерево, пока в очереди не останется один узел (корень)
        ...
        // Для специального случая, когда есть только один символ
        if (count.size() == 1) {
            // Получаем единственный символ
            Character onlyChar = count.keySet().iterator().next();
            // Если код еще не установлен, устанавливаем его в "0"
            if (!codes.containsKey(onlyChar)) {
                codes.put(onlyChar, "0");
            }
        }
    ...
    private class InternalNode extends Node {
        ...
        InternalNode(Node left, Node right) {
            super((left != null ? left.frequence : 0) + (right != null ? right.frequence : 0));
            this.left = left;
            this.right = right;
        }
