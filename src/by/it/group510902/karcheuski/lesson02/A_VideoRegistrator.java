...
public class A_VideoRegistrator {
    ...
    List<Double> calcStartTimes(double[] events, double workDuration) {
        ...
        for (int i = 0; i < sortedEvents.length - 1; i++) {
            for (int j = 0; j < sortedEvents.length - i - 1; j++) {
                if (sortedEvents[j] > sortedEvents[j + 1]) {
                    double temp = sortedEvents[j];
        ...
        while (i < sortedEvents.length) {
            // получаем первое незарегистрированное событие (по левому краю)
            double currentEvent = sortedEvents[i];

            // запоминаем время старта видеокамеры (ставим в момент события)
