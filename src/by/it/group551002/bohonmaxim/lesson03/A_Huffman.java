...
public class A_Huffman {
    ...
    String encode(InputStream inputStream) throws FileNotFoundException {
        ...
        if (root != null) {
            // Если в строке только один уникальный символ, код должен быть "0"
            if (count.size() == 1) {
                root.fillCodes("0");
            } else {
                root.fillCodes("");
        ...
        // Заполняем StringBuilder закодированными значениями из Map codes
        for (char c : s.toCharArray()) {
    ...
    abstract class Node implements Comparable<Node> {
        ...
        protected final int frequence; //частота символов
