...
public class A_Huffman {
    ...
    String encode(InputStream inputStream) throws FileNotFoundException {
        ...
        //Рассчитываем частоту каждого символа
        Map<Character, Integer> count = new HashMap<>();
        for (char c : s.toCharArray()) {
            count.put(c, count.getOrDefault(c, 0) + 1);
        }

        //Переносим все символы в приоритетную очередь в виде листьев
        ...
        //Если в строке только один уникальный символ, обрабатываем отдельно
        if (count.size() == 1) {
            ...
            //Генерация кодов из корня дерева
            Node root = priorityQueue.poll();
            if (root != null) {
                root.fillCodes("");
            }
