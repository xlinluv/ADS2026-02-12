...
public class A_QSort {
    ...
    int[] getAccessory(InputStream stream) throws FileNotFoundException {
        ...
        Event[] eventt = new Event[2 * n];
        for (int i = 0; i < n; i++) {
            eventt[2 * i] = new Event(segments[i].start, true);  // start event
            eventt[2 * i + 1] = new Event(segments[i].stop, false); // end event
        }

        Arrays.sort(eventt, (e1, e2) -> {
        ...
        for (Point point : pointObjects) {
            while (eventIndex < eventt.length && eventt[eventIndex].coordinate <= point.coordinate) {
                if (eventt[eventIndex].isStart) {
                    activeCount++;
                } else {
                    activeCount--;
                }
