...
public class A_QSort {
    ...
    int[] getAccessory(InputStream stream) throws FileNotFoundException {
        ...
        Segment[] byStart = Arrays.copyOf(segments, n);
        quicksort(byStart, 0, n - 1);

        Segment[] byStop = Arrays.copyOf(segments, n);
        Arrays.sort(byStop, Comparator.comparingInt(s -> s.stop));
        ...
        for (int i = 0; i < points.length; ++i) {
            ...
            int j = 0;
            while (j < byStart.length && byStart[j].start <= points[i]) {
                ++countStart;
                ++j;
            }

            int k = 0;
            while (k < byStop.length && byStop[k].stop < points[i]) {
                ++countStop;
                ++k;
            }
