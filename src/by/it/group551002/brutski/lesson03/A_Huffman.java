package by.it.group551002.brutski.lesson03;
...
public class A_Huffman {
    ...
    String encode(InputStream inputStream) throws FileNotFoundException {
        ...
        count.forEach((key, frequency) -> {
            LeafNode leaf = new LeafNode(count.get(key), key);
        ...
        while (priorityQueue.size() > 1) {
            ...
            A_Huffman.Node parent = new InternalNode(left, right);
            priorityQueue.add(parent);
        }

        //4. последний из родителей будет корнем этого дерева
        //это будет последний и единственный элемент оставшийся в очереди priorityQueue.
        A_Huffman.Node root_node = priorityQueue.poll();
        if (root_node != null)
            root_node.fillCodes("");
