...
public class A_Huffman {
    ...
    String encode(InputStream inputStream) throws FileNotFoundException {
        // Прочитаем строку для кодирования
        Scanner scanner = new Scanner(inputStream);
        String s = scanner.next();

        // 1. Рассчитаем частоту каждого символа
        Map<Character, Integer> count = new HashMap<>();
        for (char c : s.toCharArray()) {
            count.put(c, count.getOrDefault(c, 0) + 1);
        }

        // 2. Перенесем все символы в приоритетную очередь в виде листьев (LeafNode)
        ...
        // Извлекаем два узла с минимальной частотой, объединяем их в InternalNode и кладем обратно
        ...
        if (root != null) {
            // Обработка случая, если в строке только один вид символов (например, "aaaaa")
            if (count.size() == 1) {
                root.fillCodes("0");
            } else {
                root.fillCodes("");
