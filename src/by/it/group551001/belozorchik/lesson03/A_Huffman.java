...
public class A_Huffman {
    ...
    String encode(InputStream inputStream) throws FileNotFoundException {
        ...
        for (int i = 0; i < s.length(); i++) {
            char c = s.charAt(i); // получаем символ на позиции i
            count.put(c, count.getOrDefault(c, 0) + 1);
        }

        //2. перенесем все символы в приоритетную очередь - PriorityQueue - в виде листьев
        PriorityQueue<Node> priorityQueue = new PriorityQueue<>();
        //создали пустую очередь, в которой будут храниться узлы нашего будущего дерева - Node
        for (Map.Entry<Character, Integer> entry : count.entrySet()) {
            //берем по очереди каждую пару «буква - частота» из нашего словаря count
            // и создаем лист с этой парой - Value - Key
        ...
        while (priorityQueue.size() > 1) {  //пока не останеться один
            Node left = priorityQueue.poll();   //poll() - достает элемент с самой маленькой частотой
            Node right = priorityQueue.poll();
            priorityQueue.add(new InternalNode(left, right));
        }
        //4. последний из родителей будет корнем этого дерева
        if (!priorityQueue.isEmpty()) {
            Node root = priorityQueue.poll();   // один оставшийся - наш корень
            // Обработка крайнего случая, если строка состоит из одного уникального символа (например, "aaaa")
            if (count.size() == 1) {
                root.fillCodes("0");
            } else {
                root.fillCodes("");
        ...
        // идем по исходной строке s
        for (int i = 0; i < s.length(); i++) {
            // взяли очередную букву из текста
            char c = s.charAt(i);
            // идем в наш словарь codes и проверяем код
            String codeForChar = codes.get(c);
            // добавили код к итоговой последовательности
            sb.append(codeForChar);
        }
        return sb.toString();
        //01001100100111
        //01001100100111
