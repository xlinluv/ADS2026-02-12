...
public class A_QSort {
    ...
    int[] getAccessory(InputStream stream) throws FileNotFoundException {
        ...
        for (int i = 0; i < m; i++) {
            int x = points[i];
            // Число отрезков, у которых start <= x
            int cntStart = upperBound(segments, x);
            // Число отрезков, у которых stop < x
            int cntStop = lowerBound(stops, x);
    ...
    private void quickSortSegments(Segment[] arr, int low, int high) {
        if (low < high) {
            int p = partitionSegments(arr, low, high);
            quickSortSegments(arr, low, p - 1);
            quickSortSegments(arr, p + 1, high);
        }
    }
    ...
    private int upperBound(Segment[] seg, int x) {
