package by.it.group510901.sorokoletov.lesson04;
...
public class C_GetInversions {
    ...
    int calc(InputStream stream) throws FileNotFoundException {
        ...
        result = mergeSortCount(a, 0, n - 1);  // ← теперь возвращает int

        //!!!!!!!!!!!!!!!!!!!!!!!!!     КОНЕЦ ЗАДАЧИ     !!!!!!!!!!!!!!!!!!!!!!!!!
        return result;
    }

    // Вспомогательный метод: сортировка + подсчёт инверсий
    private int mergeSortCount(int[] arr, int left, int right) {
        if (left >= right) return 0;

        int mid = left + (right - left) / 2;
        int inv = 0;  // ← int вместо long
