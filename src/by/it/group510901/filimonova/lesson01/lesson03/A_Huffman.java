...
public class A_Huffman {
    ...
    String encode(InputStream inputStream) throws FileNotFoundException {
        ...
        //очищаем коды перед кодированием
        codes.clear();
        ...
        //особый случай: если только один символ в строке
        if (priorityQueue.size() == 1) {
            Node onlyNode = priorityQueue.poll();
            onlyNode.fillCodes("0");
        } else {
...
2. Создаем листья для каждого символа (частота, символ) и помещаем в приоритетную очередь (min-heap)
3. Пока в очереди >1 узла:
   - Извлекаем два узла с наименьшими частотами
   - Создаем родительский узел (частота = сумма)
   - Возвращаем родителя в очередь
4. Последний узел — корень дерева
5. Генерируем коды: обходим дерево, добавляя 0 при спуске влево, 1 — вправо
