package by.it.group510901.filimonova.lesson01.lesson04;

import java.io.FileNotFoundException;
import java.io.InputStream;
import java.util.Scanner;

/*
Рассчитать число инверсий одномерного массива.
Сложность алгоритма должна быть не хуже, чем O(n log n)

Первая строка содержит число 1<=n<=10000,
вторая - массив A[1…n], содержащий натуральные числа, не превосходящие 10E9.
Необходимо посчитать число пар индексов 1<=i<j<n, для которых A[i]>A[j].

    (Такая пара элементов называется инверсией массива.
    Количество инверсий в массиве является в некотором смысле
    его мерой неупорядоченности: например, в упорядоченном по неубыванию
    массиве инверсий нет вообще, а в массиве, упорядоченном по убыванию,
    инверсию образуют каждые (т.е. любые) два элемента.
    )

Sample Input:
5
2 3 9 2 9
Sample Output:
2

Головоломка (т.е. не обязательно).
Попробуйте обеспечить скорость лучше, чем O(n log n) за счет многопоточности.
Докажите рост производительности замерами времени.
Большой тестовый массив можно прочитать свой или сгенерировать его программно.
*/


public class C_GetInversions {

    public static void main(String[] args) throws FileNotFoundException {
        InputStream stream = C_GetInversions.class.getResourceAsStream("dataC.txt");
        C_GetInversions instance = new C_GetInversions();
        //long startTime = System.currentTimeMillis();
        int result = instance.calc(stream);
        //long finishTime = System.currentTimeMillis();
        System.out.print(result);
    }

    int calc(InputStream stream) throws FileNotFoundException {
        //подготовка к чтению данных
        Scanner scanner = new Scanner(stream);
        //!!!!!!!!!!!!!!!!!!!!!!!!!     НАЧАЛО ЗАДАЧИ     !!!!!!!!!!!!!!!!!!!!!!!!
        //размер массива
        int n = scanner.nextInt();
        //сам массив
        int[] a = new int[n];
        for (int i = 0; i < n; i++) {
            a[i] = scanner.nextInt();
        }

        // вызываем сортировку слиянием с подсчетом инверсий
        int result = mergeSortAndCount(a, 0, a.length - 1);

        //!!!!!!!!!!!!!!!!!!!!!!!!!     КОНЕЦ ЗАДАЧИ     !!!!!!!!!!!!!!!!!!!!!!!!!
        return result;
    }

    // рекурсивная сортировка слиянием с подсчетом инверсий
    private int mergeSortAndCount(int[] array, int left, int right) {
        int inversionCount = 0;

        if (left < right) {
            // находим середину
            int mid = left + (right - left) / 2;

            // считаем инверсии в левой и правой половинах
            inversionCount += mergeSortAndCount(array, left, mid);
            inversionCount += mergeSortAndCount(array, mid + 1, right);

            // считаем инверсии при слиянии
            inversionCount += mergeAndCount(array, left, mid, right);
        }

        return inversionCount;
    }

    // слияние двух отсортированных подмассивов с подсчетом инверсий
    private int mergeAndCount(int[] array, int left, int mid, int right) {
        // размеры временных массивов
        int n1 = mid - left + 1;
        int n2 = right - mid;

        // создаем временные массивы
        int[] leftArray = new int[n1];
        int[] rightArray = new int[n2];

        // копируем данные во временные массивы
        for (int i = 0; i < n1; i++) {
            leftArray[i] = array[left + i];
        }
        for (int j = 0; j < n2; j++) {
            rightArray[j] = array[mid + 1 + j];
        }

        // индексы для слияния
        int i = 0; // индекс левого массива
        int j = 0; // индекс правого массива
        int k = left; // индекс результирующего массива
        int inversions = 0;

        // сливаем массивы и считаем инверсии
        while (i < n1 && j < n2) {
            if (leftArray[i] <= rightArray[j]) {
                array[k] = leftArray[i];
                i++;
            } else {
                // если элемент правого массива меньше элемента левого,
                // то все оставшиеся элементы левого массива образуют инверсии с ним
                array[k] = rightArray[j];
                inversions += (n1 - i); // добавляем количество оставшихся элементов в левом массиве
                j++;
            }
            k++;
        }

        // копируем оставшиеся элементы левого массива (если есть)
        while (i < n1) {
            array[k] = leftArray[i];
            i++;
            k++;
        }

        // копируем оставшиеся элементы правого массива (если есть)
        while (j < n2) {
            array[k] = rightArray[j];
            j++;
            k++;
        }

        return inversions;
    }
}
/*1. Разделяем массив пополам, сортируем левую и правую части
2. При слиянии:
   - Когда элемент из правой части меньше элемента из левой части
   - Все оставшиеся элементы в левой части образуют инверсии с этим элементом
   - Добавляем к счетчику (n1 - i)
3. Суммируем инверсии из левой части, правой части и при слиянии

Почему работает:
- При слиянии двух отсортированных частей:
  - Если leftArray[i] > rightArray[j], то rightArray[j] меньше всех оставшихся элементов в leftArray
  - Значит, образует инверсии со всеми ними

Сложность: O(n log n)*/