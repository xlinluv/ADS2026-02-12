...
public class A_BinaryFind {
    ...
    public int[] findIndex(InputStream stream) throws FileNotFoundException {
        ...
        for (int i = 0; i < k; i++) {
            ...
            // здесь реализуйте бинарный поиск индекса

            int left = 0;       // левая граница поиска
            int right = n - 1;  // правая граница поиска
            int foundIndex = -1; // индекс найденного элемента (по умолчанию -1)

            // бинарный поиск
            while (left <= right) {
                int mid = left + (right - left) / 2; // середина текущего отрезка
            ...
            result[i] = foundIndex; // сохраняем результат
        }
        //!!!!!!!!!!!!!!!!!!!!!!!!!     КОНЕЦ ЗАДАЧИ     !!!!!!!!!!!!!!!!!!!!!!!!!
        return result;
    }
