...
public class C_GetInversions {
    ...
    int calc(InputStream stream) throws FileNotFoundException {
        ...
        return mergeSortAndCount(a, new int[n], 0, n);
    }
    ...
    private int mergeSortAndCount(int[] a, int[] temp, int left, int right) {
        ...
        int result = mergeSortAndCount(a, temp, left, middle);
        result += mergeSortAndCount(a, temp, middle, right);
        result += mergeAndCount(a, temp, left, middle, right);
        return result;
    }

    private int mergeAndCount(int[] a, int[] temp, int left, int middle, int right) {
        int inversions = 0;
        int i = left;
        int j = middle;
        int k = left;
