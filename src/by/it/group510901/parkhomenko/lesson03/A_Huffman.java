package by.it.group510901.parkhomenko.lesson03;
...
public class A_Huffman {
    ...
    String encode(InputStream inputStream) throws FileNotFoundException {
        ...
        PriorityQueue<Node> priorityQueue = new PriorityQueue<>((left, right) -> {
            int result = Integer.compare(left.frequence, right.frequence);
            if (result != 0) return result;
            return Character.compare(minSymbol(left), minSymbol(right));
        });
    ...
    private char minSymbol(Node node) {
        if (node instanceof LeafNode) return ((LeafNode) node).symbol;
        InternalNode internal = (InternalNode) node;
        return (char) Math.min(minSymbol(internal.left), minSymbol(internal.right));
    }
    ...
    private class LeafNode extends Node {
        ...
        void fillCodes(String code) {
            ...
            codes.put(this.symbol, code.isEmpty() ? "0" : code);
        }
    }
